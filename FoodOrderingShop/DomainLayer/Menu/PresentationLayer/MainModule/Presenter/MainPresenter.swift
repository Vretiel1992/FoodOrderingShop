...
class MainPresenter: MainPresenterProtocol {
    ...
    private let foodCategoryEntityMapper = FoodCategoryEntityMapper()

    private var foodCategories: [FoodCategoryModel] = [] {
        didSet {
            view?.update(with: foodCategoryMapper.map(foodCategories))
        }
    }
        ...
        menuAPIManager.getFoodCategories { [weak self] result in
            ...
            DispatchQueue.main.async {
                ...
                case .success(let foodCategoriesData):
                    self.foodCategories = self.foodCategoryEntityMapper.map(foodCategoriesData)
...
//        locationManager.updatingLocation { [weak self] locationModel in
//            DispatchQueue.main.async {
//                self?.view?.updateLocationLabel(text: locationModel.city)
//            }
//        }
